...
def generate_classification_report(df, total_samples=500):
    ...
    with open('virustotal_classification_report.txt', 'a') as report_file, open('mismatches.txt', 'a') as mismatch_file:
...
def get_page_rank(url):
    """Fetches PageRank using the OpenPageRank API."""
    ...
    try:
        ...
        print(f"Error fetching PageRank for {url}: {e}")
    return 0

def page_rank():
    df = pd.read_csv("221IT085_URLfeaturedataset.csv")
    label_index = df.columns.get_loc("Label")
    df.insert(loc=label_index, column="page_rank", value=df["URL"].apply(get_page_rank))
    df.to_csv("221IT085_URLfeaturedataset.csv", index=False)
...
def get_web_traffic(url):
    ...
    params = {"api_key": SIMILARWEB_API_KEY, "granularity": "monthly"}
    
    try:
        response = requests.get(api_url, params=params)
        data = response.json()
        if response.status_code == 200 and "visits" in data:
            return data["visits"]
    except Exception as e:
        print(f"Error fetching traffic data for {url}: {e}")
    return 0

def web_traffic():
    df = pd.read_csv("221IT085_URLfeaturedataset.csv")
    label_index = df.columns.get_loc("Label")
    df.insert(loc=label_index, column="web_traffic", value=df["URL"].apply(get_web_traffic))
    df.to_csv("221IT085_URLfeaturedataset.csv", index=False)
...
def vedic_correlation_matrix(df):
    """Computes the correlation matrix using a Vedic-inspired efficient method."""
    cols = df.columns
    n = len(df)
    means = {col: np.sum(df[col]) / n for col in cols}  # Compute mean
    std_devs = {col: np.sqrt(np.sum((df[col] - means[col])**2) / (n - 1)) for col in cols}  # Compute std dev
    correlation_matrix = pd.DataFrame(np.zeros((len(cols), len(cols))), columns=cols, index=cols)
    for i, col1 in enumerate(cols):
        for j, col2 in enumerate(cols):
            if i > j:
                continue  
            # Compute covariance using a single-pass summation (Vedic approach)
            covariance = np.sum((df[col1] - means[col1]) * (df[col2] - means[col2])) / (n - 1)
            correlation = covariance / (std_devs[col1] * std_devs[col2])
            correlation_matrix.loc[col1, col2] = correlation
            correlation_matrix.loc[col2, col1] = correlation  # Symmetric
    return correlation_matrix

def generate_heatmap(df):
    correlation_matrix = vedic_correlation_matrix(df)  # Using custom correlation function
    plt.figure(figsize=(12, 8))
    sns.heatmap(correlation_matrix, annot=True, cmap="coolwarm", fmt=".2f", cbar=True)
    plt.title("Correlation Heatmap")
    plt.savefig("221IT085-Heatmap.JPEG", format="jpeg")
    plt.close()
    print("Heatmap generated and saved.")

def plot_feature_distributions(df, nrows=2, ncols=3):
    features = df.columns
